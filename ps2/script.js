...
function time()
{
  ...
  document.getElementById("task6").innerHTML = "Между датами прошло: "
  + years + (years % 10 > 4 || years % 10 < 1 || years % 100 / 10 == 1 ? " лет":( years % 10 > 1 ? " года":" год")) + " "
  + months + (months % 10 > 4 || months % 10 < 1 || months % 100 / 10 == 1 ? " месяцев":( months % 10 > 1 ? " месяца":" месяц")) + " "
  + days + (days % 10 > 4 || days % 10 < 1 || days % 100 / 10 == 1 ? " дней":( days % 10 > 1 ? " дня":" день")) + " "
  + hours + (hours % 10 > 4 || hours % 10 < 1 || hours % 100 / 10 == 1 ? " часов":( hours % 10 > 1 ? " часа":" час")) + " "
  + seconds + (seconds % 10 > 4 || seconds % 10 < 1 || seconds % 100 / 10 == 1 ? " секунд":( seconds % 10 > 1 ? " секунды":" секунда"));
}
...
function zodiac()
{
	...
	var month = document.znak_zodiac.your_month.selectedIndex;
		...
		if (month==1 && date>=20 || month==2 && date<=18)
			document.getElementById("task7").innerHTML = "Aquarius" + '<img src="images/Aquarius.png" alt="Aquarius">';
		else if (month==2 && date>=19 || month==3 && date<=20)
			document.getElementById("task7").innerHTML = "Pisces" + '<img src="images/Pisces.jpg" alt="Pisces">';
		else if (month==3 && date>=21 || month==4 && date<=19)
			document.getElementById("task7").innerHTML = "Aries" + '<img src="images/Aries.jpg" alt="Aries">';
		else if (month==4 && date>=20 || month==5 && date<=20)
			document.getElementById("task7").innerHTML = "Taurus" + '<img src="images/Taurus.jpg" alt="Taurus">';
		else if (month==5 && date>=21 || month==6 && date<=21)
			document.getElementById("task7").innerHTML = "Gemini" + '<img src="images/Gemini.jpg" alt="Gemini">';
		else if (month==6 && date>=22 || month==7 && date<=22)
			document.getElementById("task7").innerHTML = "Cancer" + '<img src="images/Cancer.jpg" alt="Cancer">';
		else if (month==7 && date>=23 || month==8 && date<=22)
			document.getElementById("task7").innerHTML = "Leo" + '<img src="images/Leo.jpg" alt="Leo">';
		else if (month==8 && date>=23 || month==9 && date<=22)
			document.getElementById("task7").innerHTML = "Virgo" + '<img src="images/Virgo.jpg" alt="Virgo">';
		else if (month==9 && date>=23 || month==10 && date<=22)
			document.getElementById("task7").innerHTML = "Libra" + '<img src="images/Libra.jpg" alt="Libra">';
		else if (month==10 && date>=23 || month==11 && date<=21)
			document.getElementById("task7").innerHTML = "Scorpio" + '<img src="images/Scorpio.jpg" alt="Scorpio">';
		else if (month==11 && date>=22 || month==12 && date<=21)
			document.getElementById("task7").innerHTML = "Sagittarius" + '<img src="images/Sagittarius.jpg" alt="Sagittarius">';
		else if (month==12 && date>=22 || month==1 && date<=19)
			document.getElementById("task7").innerHTML = "Capricorn" + '<img src="images/Capricorn.jpg" alt="Capricorn">';
