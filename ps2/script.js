...
function age()
{
	var n = Number(document.getElementById("age").value);
	  //ternary operator
  document.getElementById("task5").innerHTML = "Мне " + n + (n % 10 > 4 || n % 10 < 1 
                                                          || n % 100 / 10 == 1 ? " лет":
                                                           ( n % 10 > 1 ? " года":" год"));
}
...
function time()
{
  ...
  if (!date1.getTime() || !date2.getTime()) {
    ...
    document.getElementById("task6").innerHTML = "Пожалуйсте, будьте так добры и заполните все поля";
    return false;
  } else if (interval < 0) {

    document.getElementById("task6").innerHTML = "Первая дата должна быть меньше второй";
    return false;
  }
  ...
  seconds %= 60;
  var arr = [];
  arr.push(years, months, days, hours, seconds);

  document.getElementById("task6").innerHTML = "Между датами прошло: "
  + years + (years % 10 > 4 || years % 10 < 1 || years % 100 / 10 == 1 ? " лет":
    ( years % 10 > 1 ? " года":" год")) + " "
  + months + (months % 10 > 4 || months % 10 < 1 || months % 100 / 10 == 1 ? " месяцев":
    ( months % 10 > 1 ? " месяца":" месяц")) + " "
  + days + (days % 10 > 4 || days % 10 < 1 || days % 100 / 10 == 1 ? " дней":
    ( days % 10 > 1 ? " дня":" день")) + " "
  + hours + (hours % 10 > 4 || hours % 10 < 1 || hours % 100 / 10 == 1 ? " часов":
    ( hours % 10 > 1 ? " часа":" час")) + " "
  + seconds + (seconds % 10 > 4 || seconds % 10 < 1 || seconds % 100 / 10 == 1 ? " секунд":
    ( seconds % 10 > 1 ? " секунды":" секунда"));
}
...
function board()
{
  ...
  if (n1 < 0 || n2 < 0 || n > 50 || n > 50 || isNaN(n1) || isNaN(n2)) {
    document.getElementById("task8").innerHTML = "Пожалуйста, введите числа больше 0 и меньше 50";
    return false;
  } else if (!n1 || !n2) {
    document.getElementById("task8").innerHTML = "Пожалуйста, заполните оба поля";
    return false;
  }
...
function flat() {
    ...
    if (number <= 0 || number > (floors * flatsPerFloors * entrances)) {
    document.getElementById("task9").innerHTML = "Некорректный номер квартиры";
    return false;
    } else if (isNaN(number) || isNaN(entrances) || isNaN(flatsPerFloors) || isNaN(floors)) {
    document.getElementById("task9").innerHTML = "Введены некорректные данные";
    return false;
    }
    ...
    document.getElementById("task9").innerHTML =  "Этаж: " + floor + "Номер парадной: " + entrance;
}
...
function num_sum() {
  ...
  if(isNaN(n)) {
    document.getElementById("task10").innerHTML = "Введено некоректное число";
    return false;
  }
